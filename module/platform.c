...
#define spi_wait(); while (!spi_xfer_done) {}
...
void spi_config_oled(void)
{
	...
	err_code = nrf_drv_spi_init(&m_spi_master_0, &config, spi_event_handler, NULL);
	APP_ERROR_CHECK(err_code);
	is_spi_conf = 3;
}
...
void shutdown_routine(void)
{
	...
	nrf_gpio_input_disconnect(OLED_CS_PIN);
