...
#define APP_ADV_FAST_INTERVAL            MSEC_TO_UNITS(100,UNIT_0_625_MS)         /**< Fast advertising interval (in units of 0.625 ms. This value corresponds to 25 ms.). */
#define APP_ADV_SLOW_INTERVAL            MSEC_TO_UNITS(250,UNIT_0_625_MS)        /**< Slow advertising interval (in units of 0.625 ms. This value corrsponds to 2 seconds). */
...
#define MIN_CONN_MS 50
#define MAX_CONN_MS 100
...
// ---------------blackbox start------------------
#define BB_DEVICE_NAME                     "BlackBox"

#define BB_APP_ADV_FAST_INTERVAL            MSEC_TO_UNITS(75,UNIT_0_625_MS)
#define BB_APP_ADV_SLOW_INTERVAL            MSEC_TO_UNITS(125,UNIT_0_625_MS)
#define BB_APP_ADV_FAST_TIMEOUT             30
#define BB_APP_ADV_SLOW_TIMEOUT             0

#define BB_MIN_CONN_INTERVAL               MSEC_TO_UNITS(15, UNIT_1_25_MS)
#define BB_MAX_CONN_INTERVAL               MSEC_TO_UNITS(50, UNIT_1_25_MS)
#define BB_SLAVE_LATENCY                   0
#define BB_CONN_SUP_TIMEOUT                MSEC_TO_UNITS(2000, UNIT_10_MS)
// ---------------blackbox end------------------
...
BLE_CENTRAL_USER_DEF(m_ble_central);
...
// blackbox ver
static void bb_gap_params_init(void)
{
	uint32_t                err_code;
	ble_gap_conn_params_t   gap_conn_params;
	ble_gap_conn_sec_mode_t sec_mode;
	...
	gap_conn_params.min_conn_interval = BB_MIN_CONN_INTERVAL;
	gap_conn_params.max_conn_interval = BB_MAX_CONN_INTERVAL;
	gap_conn_params.slave_latency     = BB_SLAVE_LATENCY;
	gap_conn_params.conn_sup_timeout  = BB_CONN_SUP_TIMEOUT;
...
static void user_data_handler(ble_user_evt_t* p_evt)
{
	if (p_evt->type == BLE_USER_EVT_RX_DATA) {
		static uint8_t p_data[256];
		static uint16_t p_length;
		static ble_user_data_t p_user_data;
		// uint32_t err_code;
		NRF_LOG_DEBUG("Received data from BLE.");
		NRF_LOG_HEXDUMP_DEBUG(p_evt->params.rx_data.p_data, p_evt->params.rx_data.length);
		memcpy(p_data, p_evt->params.rx_data.p_data, p_evt->params.rx_data.length);
		p_user_data.p_data = p_data;
		p_length = p_evt->params.rx_data.length;
		p_user_data.p_length = &p_length;
		uevt_bc(UEVT_BT_DATARECV, &p_user_data);
	}
}
	...
	switch (p_ble_evt->header.evt_id) {
		...
		case BLE_GAP_EVT_CONNECTED: {
			...
			if (m_gap_role == BLE_GAP_ROLE_PERIPH) {
				...
				// 如果是主机模式
				NRF_LOG_RAW_INFO("Connected to target\n");
				err_code = ble_user_handles_assign(&m_ble_central, p_gap_evt->conn_handle, NULL);
				APP_ERROR_CHECK(err_code);
				// g_conn_handle = p_gap_evt->conn_handle;
...
static void services_init(void)
{
	...
	ble_central_user_init_t central_user_init;
	...
	memset(&central_user_init, 0, sizeof(central_user_init));
	central_user_init.data_handler = user_data_handler;
	central_user_init.evt_handler = ble_central_user_evt_handler;
	err_code = ble_central_user_init(&m_ble_central, &central_user_init);
	APP_ERROR_CHECK(err_code);
...
// blackbox ver
static void bb_services_init(void)
{
	uint32_t           err_code;
	ble_user_init_t     user_init;
...
// blackbox ver
static void bb_advertising_init(void)
{
	uint32_t               err_code;
	ble_advertising_init_t init;
	...
	init.config.ble_adv_fast_enabled  = true;
	init.config.ble_adv_fast_interval = BB_APP_ADV_FAST_INTERVAL;
	init.config.ble_adv_fast_timeout  = BB_APP_ADV_FAST_TIMEOUT;
	init.config.ble_adv_slow_enabled  = true;
	init.config.ble_adv_slow_interval = BB_APP_ADV_SLOW_INTERVAL;
	init.config.ble_adv_slow_timeout  = BB_APP_ADV_SLOW_TIMEOUT;
	init.evt_handler = on_adv_evt;
...
{
	ble_user_on_db_disc_evt(&m_ble_central, p_evt);
}
...
static void db_discovery_init(void)
{
	...
	APP_ERROR_CHECK(err_code);
	err_code = ble_db_discovery_evt_register(&m_user_uuid);
	APP_ERROR_CHECK(err_code);
}
	...
	switch(evt->evt_id) {
		...
		case UEVT_BT_CTL_DATASEND:
			ble_user_send(&m_user,
			              ...
			              m_conn_handle);
			break;
		case UEVT_BT_CTL_CENTRAL_DATASEND:
			ble_central_user_send(&m_ble_central,
			                      ((ble_user_data_t*)(evt->content))->p_data,
			                      ((ble_user_data_t*)(evt->content))->p_length
			                     );
			break;
